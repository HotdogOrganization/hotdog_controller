...
**Warning**： this control algorithm was only fully tested on Hotdog2. Regarding Hotdog1, not all functionalities are guaranteed, so please be cautious to use!
...
```
Above is for checking if the code is integral and compilable. To run simulation in Gazebo, the whole [hotdog_sim](https://github.com/MiRoboticsLab/hotdog_sim) repository should be downloaded which already includes this code inside and compiled in ROS2 environment. Please refer to the ReadMe of the [hotdog_sim](https://github.com/MiRoboticsLab/hotdog_sim) project for more details:
```
...
Then, the BUILD_ROS should be set to ON in src/hotdog_locomotion/CMakeLists.txt, and all the project can be compiled in the directory 'hotdog_sim':
```
...
$ cd onboard-build
```
**Note**: your_own_path/hotdog_locomotion needs to be an absolute path.
- For Hotdog:
```
...
$ make -j4
```
After successful compilation, connect the external PC to the Ethernet port of robot(Hotdog2) or Download USB port(Hotdog)，and then send the compiled product to robot：
```
