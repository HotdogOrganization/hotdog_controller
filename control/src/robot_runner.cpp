...
void RobotRunner::InitTask() {
    ...
    if ( robot_type_ == RobotType::CYBERDOG || robot_type_ == RobotType::CYBERDOG2 ) {
        quadruped_ = BuildHotdog< float >( robot_type_, robot_appearance_type_ );
    }
    else {
        assert( false );
    }
...
void RobotRunner::RunTask() {
    ...
    //         hotdog2_main_visualization_->q[ leg * 3 + joint ] = leg_controller_->datas_[ leg ].q[ joint ];
    //     }
    // }
    // hotdog2_main_visualization_->p    = state_estimate_result_.position;
    // hotdog2_main_visualization_->quat = state_estimate_result_.orientation;
