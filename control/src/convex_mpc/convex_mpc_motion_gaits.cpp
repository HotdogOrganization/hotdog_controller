...
void ConvexMpcMotionGaits::SetCmd( ControlFsmData< float >& data ) {
    ...
    // set vel_des_
    vel_cmd_( 0 ) += ( ( duration_mode_ || ctrl_cmd_->cmd_source > kHotdogLcmCmd ) ? 1.0 : vel_cmd_max_[ 0 ] ) * ctrl_cmd_->vel_des[ 0 ];  // RC and GAMEMPAD are same
    vel_cmd_( 1 ) += ( ( duration_mode_ || ctrl_cmd_->cmd_source > kHotdogLcmCmd ) ? 1.0 : vel_cmd_max_[ 1 ] ) * ctrl_cmd_->vel_des[ 1 ];
    vel_cmd_( 2 ) += ( ( duration_mode_ || ctrl_cmd_->cmd_source > kHotdogLcmCmd ) ? 1.0 : vel_cmd_max_[ 2 ] ) * ctrl_cmd_->vel_des[ 2 ];
