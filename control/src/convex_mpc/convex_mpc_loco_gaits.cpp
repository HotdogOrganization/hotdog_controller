...
void ConvexMpcLocoGaits::SwitchByPercentage() {
    ...
    // Hotdog kTrot10v4 is better but Hotdog2 is reversed
...
void ConvexMpcLocoGaits::SetCmd( ControlFsmData< float >& data ) {
    ...
    else if ( duration_mode_ || ctrl_cmd_->cmd_source > kHotdogLcmCmd ) {
        for ( int i = 0; i < 3; i++ ) {
    ...
    if ( ctrl_cmd_->cmd_source == kHotdog2LcmCmd || ctrl_cmd_->cmd_source == kHotdogLcmCmd ) {
