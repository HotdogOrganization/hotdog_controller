...
WbcCtrl< T >::WbcCtrl( FloatingBaseModel< T > model )
    : full_config_( hotdog2::kNumActJoint + 7 ), tau_ff_( hotdog2::kNumActJoint ), des_jpos_( hotdog2::kNumActJoint ), des_jvel_( hotdog2::kNumActJoint ), wbc_lcm_( GetLcmUrl( 255 ) ) {
    iter_ = 0;
    full_config_.setZero();

    model_   = model;
    kin_wbc_ = new KinWbc< T >( hotdog2::kDimConfig );

    wbic_      = new WBIC< T >( hotdog2::kDimConfig, &( contact_list_ ), &( task_list_ ) );
    ...
    kp_joint_.resize( hotdog2::kNumLegJoint, 5. );
    kd_joint_.resize( hotdog2::kNumLegJoint, 1.5 );

    state_.q  = DVec< T >::Zero( hotdog2::kNumActJoint );
    state_.qd = DVec< T >::Zero( hotdog2::kNumActJoint );
}
...
template < typename T > void WbcCtrl< T >::UpdateLegCmd( ControlFsmData< T >& data ) {
    ...
    for ( size_t leg( 0 ); leg < hotdog2::kNumLeg; ++leg ) {
        cmd[ leg ].Zero();
        for ( size_t jidx( 0 ); jidx < hotdog2::kNumLegJoint; ++jidx ) {
            cmd[ leg ].tau_feed_forward[ jidx ] = tau_ff_[ hotdog2::kNumLegJoint * leg + jidx ];
            cmd[ leg ].q_des[ jidx ]            = des_jpos_[ hotdog2::kNumLegJoint * leg + jidx ];
            cmd[ leg ].qd_des[ jidx ]           = des_jvel_[ hotdog2::kNumLegJoint * leg + jidx ];
